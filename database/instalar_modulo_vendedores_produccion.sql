-- =====================================================
-- INSTALACIÓN COMPLETA MÓDULO VENDEDORES EN PRODUCCIÓN
-- =====================================================
-- IMPORTANTE: Ejecutar estos scripts EN ORDEN
-- =====================================================

-- PASO 1: Corregir trigger defectuoso
-- =====================================================
...
-- PASO 2: Crear todas las tablas del módulo vendedores
-- =====================================================
...
CREATE TABLE IF NOT EXISTS `comisiones` (
  ...
  KEY `idx_vendedor_estado` (`vendedor_id`, `estado`),
...
CREATE TABLE IF NOT EXISTS `pagos_comisiones` (
  ...
  KEY `idx_fecha_metodo` (`fecha_pago`, `metodo_pago`),
    ...
    IF NEW.estado = 'vendido' AND OLD.estado != 'vendido' AND NEW.vendedor_id IS NOT NULL THEN
        ...
        IF v_vendedor_id IS NOT NULL THEN
            ...
            INSERT INTO comisiones (
                ...
                NEW.id, v_vendedor_id, v_valor_venta,
                v_porcentaje, v_valor_comision,
                'pendiente', COALESCE(NEW.fecha_venta, CURDATE())
            );
        END IF;
    END IF;
END$$
...
BEGIN
    ...
    FROM comisiones WHERE id = NEW.comision_id;
    
    SELECT COALESCE(SUM(valor_pagado), 0) INTO v_total_pagado
    FROM pagos_comisiones WHERE comision_id = NEW.comision_id;
    
    IF v_total_pagado >= v_valor_comision THEN
        UPDATE comisiones SET estado = 'pagada' WHERE id = NEW.comision_id;
    ELSEIF v_total_pagado > 0 THEN
        UPDATE comisiones SET estado = 'pagada_parcial' WHERE id = NEW.comision_id;
    END IF;
END$$
DELIMITER ;
...
BEGIN
    IF OLD.porcentaje_comision_default != NEW.porcentaje_comision_default THEN
        INSERT INTO historial_comisiones (
            vendedor_id, porcentaje_anterior, porcentaje_nuevo, fecha_cambio
        ) VALUES (
            NEW.id, OLD.porcentaje_comision_default, NEW.porcentaje_comision_default, NOW()
        );
    END IF;
END$$
DELIMITER ;
...
INSERT INTO vendedores (
    user_id, codigo_vendedor, numero_documento,
    nombres, apellidos, email,
    porcentaje_comision_default, fecha_ingreso, estado
)
SELECT 
    u.id,
    CONCAT('VEND-', LPAD(u.id, 4, '0')),
    CONCAT('DOC-', u.id),
...
-- =====================================================
-- FIN - Módulo instalado correctamente
-- =====================================================
